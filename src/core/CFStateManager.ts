...
export class CFStateManager{
    ...
    static async guardInitialized():Promise<void>{
        ...
        const globalUri = vscode.Uri.joinPath(cfRoot, "global.json");

        const currentExists = await FileSystemService.exists(currentUri);
        const globalExists = await FileSystemService.exists(globalUri);
        if(!currentExists || !globalExists){
            throw new Error("ContextForge is not initialized. Please run the 'Initialize ContextForge' command first.");
        }   
    }
    static async getActiveVersion():Promise<CurrentConfig>{
        ...
        try{
            ...
            if(typeof current.latestVersion !== "number"){
                throw new Error("current json has missing latestVersion or it is not a number");
            }
            this.currentCache = current;
            return current;
        }
        catch(error){
            throw new Error(`Failed to load current configuration. ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    static getTempPath(uri:vscode.Uri):vscode.Uri{
    ...
    static async getGlobal():Promise<GlobalConfig>{
        ...
        const activeVersionPath= this.getVersionPath(current.activeVersion);
        const globalUri=vscode.Uri.joinPath(activeVersionPath, "global.json");
    ...
    try {
      ...
      throw new Error(
        `Failed to update global.json for active version "${current.activeVersion}". ${this.getErrorMessage(error)}`
      );
    }
  }
